...
TEST_CASE("test fd_stat") {
  ...
  CHECK(local->get_stat() == channel::stat_t::open);
  local->shutdown_read();
  local->shutdown_read();
  CHECK(local->get_stat() == channel::stat_t::shutdown_read);
  local->shutdown_write();
  local->shutdown_write();
  CHECK(local->get_stat() == channel::stat_t::shutdown_rw);
  local->force_close();
  local->shutdown_rw();
  CHECK(local->get_stat() == channel::stat_t::closed);

  // shutdown rw -> close
  auto peer = std::make_shared<channel>(peer_fd, loop);
  peer->shutdown_rw();
  CHECK(peer->get_stat() == channel::stat_t::shutdown_rw);
  peer->force_close();
  CHECK(peer->get_stat() == channel::stat_t::closed);

  // shutdown write -> shutdown read -> close
  auto peer2 = std::make_shared<channel>(peer_fd2, loop);
  peer2->shutdown_write();
  CHECK(peer2->get_stat() == channel::stat_t::shutdown_write);
  peer2->shutdown_read();
  CHECK(peer2->get_stat() == channel::stat_t::shutdown_rw);
  peer2->force_close();
  CHECK(peer2->get_stat() == channel::stat_t::closed);
